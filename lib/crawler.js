...
Crawler.prototype.fetch = function(now) {
  if (!now) {
    // This simulate the default parameter value
    now = (Date.now() / 1000).toFixed();
  }
  
  var httpOptions = this.createHTTPOptions(now),
      

  this.req = http.get(httpOptions, this.httpHandeler);
  this.req.end();
}
...
Slideshare.prototype.timeHash = function(now) {
  this.crypto.update(config.slideshare_secret + now);
  return this.crypto.digest('hex');
};
