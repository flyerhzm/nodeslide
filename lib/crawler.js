...
var fetchSlides = function(tag_name) {
  ...
  var xml_handler = new htmlparser.DefaultHandler(function(err, dom) {
    if (err) {
      ...
      for (var index = 0, length = slideshow_doms.length; index < length; index++) {
...
var fetchEmbed = function(dom) {
  ...
  for (var index = 0, length = data_doms.length; index < length; index++) {
...
var fetchDomData = function(dom, name) {
  var data_dom = DomUtils.getElementsByTagName(name, dom),
    data = "";
  var children = data_dom[0]["children"];
  if (children) {
...
var fetchDomArray = function(dom, name) {
  ...
  for (var index = 0, length = data_dom.length; index < length; index += 1) {
    var child_dom = data_dom[index];
    var children = child_dom["children"];
    if (children) {
      data.push(children[0]["data"]);
    }
  }
  return data;
};
